...
class Tracking(object):
    ...
    supported_backend = ["wandb", "mlflow", "swanlab", "console", "tensorboard", "rl_logging_board"]
    ...
    def log(self, data, step, batch, backend=None, tokenizer=None, rm_response_list=None):
        for default_backend, logger_instance in self.logger.items():
            if backend is None or default_backend in backend:
                if default_backend == 'rl_logging_board' and (rm_response_list is not None):
                    logger_instance.log(data=data, step=step, batch=batch, tokenizer=tokenizer, rm_response_list=rm_response_list)
                else:
